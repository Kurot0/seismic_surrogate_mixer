    ...
    def _fix_weight_norm_deepcopy(self):
        ...
        def __deepcopy__(self, memo):
            ...
            for hook in self._forward_pre_hooks.values():
                if isinstance(hook, WkeightNorm):
...
class Network(nn.Module):
    def __init__(self, modes_x, modes_y, embed_dim, img_size_out, in_chans, n_layers, share_weight, factor,
                 ff_weight_norm, n_ff_layers, layer_norm, **kwargs):
        super().__init__()
        ...
        self.in_proj = WNLinear(in_chans, self.embed_dim, wnorm=ff_weight_norm)
        ...
        if share_weight:
            self.fourier_weight = nn.ParameterList([])
            for n_modes in [modes_x, modes_y]:
                weight = torch.FloatTensor(embed_dim, embed_dim, n_modes, 2)
                param = nn.Parameter(weight)
                nn.init.xavier_normal_(param)
                self.fourier_weight.append(param)
        ...
        for _ in range(n_layers):
            self.spectral_layers.append(SpectralConv2d(in_dim=embed_dim,
                                                       out_dim=embed_dim,
                                                       modes_x=modes_x,
                                                       modes_y=modes_y,
                                                       forecast_ff=None,
        ...
        self.out = nn.Sequential(
            WNLinear(self.embed_dim, 128, wnorm=ff_weight_norm),
