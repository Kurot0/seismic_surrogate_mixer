...
class SpectralConv2d(nn.Module):
    """Factorized 2‑D spectral convolution (X then Y)."""
    ...
    def forward(self, x):
        # reshape to channels‑first for FFT
...
class ChannelWiseProjection(nn.Module):
    ...
    def forward(self, x):
        outs = [proj(x) for proj in self.projs]  # list of [B,H,W,1]
        out = torch.cat(outs, dim=-1)            # [B,H,W,out_chans]
...
class Network(nn.Module):
    def __init__(
        ...
        *,
        ...
        width: int,
        img_size_out: tuple,
        input_dim: int,  # channel of x
        underground_channels,  # channel of underground_data
        n_layers: int,
        mlp_factor: int = 2,
        out_chans: int = 1,
        **kwargs,
    ...
    def _get_grid(self, shape, device):
        ...
        gx = gx.repeat(B, 1, Y, 1)  # (B,X,Y,1)
        gy = gy.repeat(B, X, 1, 1)  # (B,X,Y,1)
        return torch.cat((gx, gy), dim=-1)

    def _encode(self, x, proj, resize: bool):
        """Common encoding: (optional resize) + coord concat + linear embed + pad."""
        if resize:
        ...
        x = torch.cat((x, grid), dim=-1)  # add coords => C+2
        x = proj(x)  # linear -> width
        x = x.permute(0, 3, 1, 2)  # B C X Y (C=width)
        x = F.pad(x, [0, self.padding, 0, self.padding])
        x = x.permute(0, 2, 3, 1)  # B X Y C
        return x

    def forward(self, x, underground, **kwargs):
        g = underground.expand(x.shape[0], -1, -1, -1)  # match batch size
        ...
        # fusion
        z = self.fusion(a, g)  # [B,H,W,width*3]
