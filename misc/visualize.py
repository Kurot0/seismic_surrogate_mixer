...
def infer_default_save_dir_from_pred_path(pred_data_path: str):
    ...
    return os.path.join(exp_root, "images")
...
def save_images(data_path, save_dir, mask_path, apply_mask, show_colorbar, model_name, scenario_names):
    ...
    for i in range(batch_size):
        ...
        concat_img.save(os.path.join(concat_dir, f"{model_name}_{scenario}_concat.png"))
...
def save_scatter_plots(true_data_path, pred_data_path, mask_path, save_dir, model_name, scenario_names):
    ...
    for i in range(batch_size):
        ...
        concat_scatter.save(os.path.join(concat_dir, f"{model_name}_{scenario}_concat_sp.png"))
...
def main():
    ...
    save_images(pred_data_path, save_dir, args.mask_path, args.apply_mask, args.show_colorbar, model, scenario_names)
    save_scatter_plots(true_data_path, pred_data_path, args.mask_path, save_dir, model, scenario_names)
